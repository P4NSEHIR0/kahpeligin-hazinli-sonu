...
module.exports.run = async (client, message, args, durum, kanal) => {
    ...
    await message.react(`${client.emojis.cache.find(x => x.name === "meri_unmute")}`);
    await message.react(`${client.emojis.cache.find(x => x.name === "meri_vunmute")}`);
    const filter = (reaction, user) => {return ["meri_unmute", "meri_vunmute"].includes(reaction.emoji.name) && user.id === message.author.id;};
    ...
    collector.on("collect", async (reaction) => {
        ...
        if(reaction.emoji.name == "meri_unmute") {
        ...
        } else if (reaction.emoji.name == "meri_vunmute") {
