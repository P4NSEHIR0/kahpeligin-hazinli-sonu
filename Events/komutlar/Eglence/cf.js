...
module.exports.run = async (client, message, args,durum, kanal) => {
	...
	if (sec == "all") {
        ...
        return message.channel.send(`**${message.author.username}**, **${data.para <= 50000 ? data.para : 50000}** :dollar: para harcadı.\nMadeni para dönüyor... ${client.emojis.cache.find(x => x.name === "meri_coin")}`).then(async mesaj => setTimeout(async () => {
            if (durums === "Tebrikler Kazandın") {
                ...
                mesaj.edit(`**${message.author.username}**, **${data.para <= 50000 ? data.para : 50000}** :dollar: para harcadı.\nMadeni para dönüyor... ${client.emojis.cache.find(x => x.name === "meri_coin")} ve 2 katını kazandın!**`)
            } else {
                mesaj.edit(`**${message.author.username}**, **${data.para <= 50000 ? data.para : 50000}** :dollar: para harcadı.\nMadeni para dönüyor... ${client.emojis.cache.find(x => x.name === "meri_coin")} tüm paranı kaybettin... :c`)
            }
        }, 5000))
	}
    ...
    if (data.para > Number(args[0]) && Number(args[0]) <= 50000 && Number(args[0]) > 0) {
        ...
        message.channel.send(`**${message.author.username}**, **${sec}** :dollar: para harcadı.\nMadeni para dönüyor... ${client.emojis.cache.find(x => x.name === "coinflip")}`).then(async mesaj => setTimeout(async () => {
            if (durums === "Tebrikler Kazandın") {
                ...
                mesaj.edit(`**${message.author.username}**, **${sec}** :dollar: para harcadı.\nMadeni para dönüyor... ${client.emojis.cache.find(x => x.name === "meri_coin")} ve 2 katını kazandın!**`)
            } else {
                mesaj.edit(`**${message.author.username}**, **${sec}** :dollar: para harcadı.\nMadeni para dönüyor... ${client.emojis.cache.find(x => x.name === "meri_coin")} tüm paranı kaybettin... :c`)
            }
        }, 5000))
