...
module.exports = async member => {
  ...
  if (guvenilirlik) {
    ...
    await member.roles.set(kayitsizRol2).then(async () => {
      await member.setNickname(`${tag2} Kayıtsız`)
      kanalKontrol.get(kayitKanal).send(`
${client.emojis.cache.find(x => x.name === "meri_yildiz")} **${member.guild.name}** sunucumuza hoş geldin
${member} (\`${member.id}\`) seninle sunucumuz **${member.guild.memberCount}** üye sayısına ulaştı.\n
Hesabın **${member.user.createdAt.toTurkishFormatDate()}** tarihinde oluşturulmuş.\nSol taraftaki __Confirmed__ odalarından birisine girerek kayıt olabilirsin! 
\`\`\`
Kayıt olduktan sonra rules kanalını okuduğunuzu kabul edeceğiz ve içeride yapılacak cezalandırma işlemlerini bunu göz önünde bulundurarak yapacağız.
\`\`\`
`).catch(console.error);
